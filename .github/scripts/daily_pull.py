...
- **أغلب المنشورات لا تستحق بطاقة.** أسقط: التهاني، النكات،
...
2. **أداة يستعملها** — الأداة **مذكورة بالاسم في قائمة أدوات عزيز أدناه**،
   والخبر يمسّها هي: ميزة · تسعير · حدّ استخدام · تعطّل · ثغرة أمنية · طريقة استعمال.
   **القائمة حرفية لا استنتاجية:** منتجٌ ليس فيها لا يدخل هذه الدرجة مهما أشبه
   أدواته أو نفعه. «Respan تطلق نموذج P-1 لحذف البيانات الحساسة» = نموذج،
   لأن Respan ليست في القائمة — وإن كانت أداة مفيدة.
...
6. **خارج الاهتمام** — **قائمة مغلقة، لا حكم عام على قلّة الأهمية:**
   استحواذ · تمويل · تقييم شركات · أسهم · اكتتاب · دعاوى · تعيينات · صراعات شركات.
   ما لا يقع في واحدة من هذه الثمانية **لا يدخل هذه الدرجة** — وأدنى ما يهبط
   إليه «عالم AI عام».
   **ويُستثنى فيرتفع:** التسعير · الإيقاف والتعطّل · أمن أداةٍ يستعملها.
   «Anthropic تُقيَّم بتريليونين» = خارج الاهتمام · «ثغرة في Claude Code» =
   أداة يستعملها · «انقطاع Grok بعطل في مركز بيانات» = **عالم AI عام**،
   فالتعطّل مستثنى صراحةً وGrok ليست في قائمة الأدوات.
